...
function LinhaParametro({
  ...
  return (
    <li
      className={`rounded-sm border p-3 ${foraPadrao ? "border-red-300 bg-red-50" : "border-gray-200 bg-white"}`}
    >
      <div className="flex items-center justify-between gap-2">
        <span className="font-medium text-gray-900">{parametro.nome}</span>
        {foraPadrao && (
          <span className="rounded-sm bg-red-100 px-2 py-0.5 text-xs font-semibold uppercase tracking-wide text-red-800">
            Fora do padrão
          </span>
        )}
      </div>
